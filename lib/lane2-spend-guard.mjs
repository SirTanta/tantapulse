...
export async function checkAndReserveApifyRun({
  ...
  // Apify returns usage in 'cents' (or its own credit unit); normalise to USD.
  // Known fields: estimatedUsage, usageEntry, availableCredits, etc.
  const rawCurrent = limitsData?.currentUsageUsd
    ?? limitsData?.estimatedUsage
    ?? limitsData?.usage
    ?? 0;
  const budgetCurrentUsd = Number(rawCurrent) || 0;

  // availableCredits may be in different units; if it looks like cents, divide.
  const rawAvailable = limitsData?.availableCredits ?? 0;
  let budgetRemainingUsd = Number(rawAvailable) || 0;
  if (budgetRemainingUsd > 100_000) {
    // Likely in cents — Apify sometimes reports cents even in USD context.
    budgetRemainingUsd = budgetRemainingUsd / 100;
  }
