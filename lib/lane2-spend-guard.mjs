...
export async function checkAndReserveApifyRun({
  ...
  // When maxMonthlyUsageUsd is 0/null/undefined (usage-based billing with
  // no hard cap), fall back to the env-configured budgetCap.
  const budgetCurrentUsd  = Number(limitsData?.current?.monthlyUsageUsd ?? 0);
  const apifyPlanLimit    = Number(limitsData?.limits?.maxMonthlyUsageUsd ?? 0);
  const maxBudgetUsd      = apifyPlanLimit > 0 ? apifyPlanLimit : budgetCapUsd;
